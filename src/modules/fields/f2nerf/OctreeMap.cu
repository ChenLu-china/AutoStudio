...
void __device__ QueryFrameTransform(const OctreeTransInfo& trans,
                                    const Wec3f& cur_xyz,
                                    Wec3f* fill_xyz) {
  Wec4f cur_xyz_ext;
...
void __device__ QueryFrameTransformJac(const OctreeTransInfo& trans,
                                       const Wec3f& cur_xyz,
                                       Watrix33f* jac) {
...
__global__ void RayMarchKernel(int n_rays, float sample_l, bool scale_by_dis,
                               ...
                               OctreeNode* octree_nodes, OctreeTransInfo* octree_transes,
...
SampleResultFlex OctreeMap::GetSamples(const Tensor& rays_o_raw, const Tensor& rays_d_raw, const Tensor& bounds_raw) 
{
    ...
    CK_CONT(stack_info);
    CK_CONT(bounds);
    CK_CONT(octree_->octree_nodes_gpu_);
    CK_CONT(octree_->octree_trans_gpu_);
...
void OctreeMap::UpdateOctNodes(const SampleResultFlex& sample_result,
                                 ...
                                 const Tensor& sampled_alpha) 
{
    ...
    const int n_nodes = octree_->octree_nodes_.size();
    ...
    Tensor& visit_cnt = octree_->tree_visit_cnt_;
    ...
    Tensor& node_weight_stats = octree_->tree_weight_stats_;
    ...
    Tensor& node_alpha_stats = octree_->tree_alpha_stats_;
