...
__global__ void SHKernel(const uint32_t n_pts, 
                    const uint32_t degree, 
                    float* dirs, 
                    float* out)
{
    const uint32_t id = threadIdx.x + blockDim.x * blockIdx.x;
    if (id >= n_pts) return;

    out = out + (degree * degree) * id;
...
Tensor SHShader::SHEncode(const Tensor& dirs)
{
    ...
    SHKernel<<<grid_dim, block_dim>>>(n_pts, degree_, dirs.data_ptr<float>(), out.data_ptr<float>());
    return out;
}
