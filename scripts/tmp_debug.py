...
def make_image_list(data_path, cam_list, factor):
    ...
    for cam in cam_list:
        ...
        if 0.999 < factor < 1.001:
            for suf in suffix:
                image_list += glob(os.path.join(data_path, cam, 'images', suf)) +\
                            glob(os.path.join(data_path, cam, 'images_1', suf))
        else:
            f_int = int(np.round(factor))
            for suf in suffix:
                image_list += glob(os.path.join(data_path, cam, 'images_{}'.format(f_int), suf))

        assert len(image_list) > 0, "No image found"
        image_list.sort()

        f = open(os.path.join(data_path, cam, 'image_list.txt'), 'w')
        for image_path in image_list:
            f.write(image_path + '\n')


@hydra.main(version_base=None, config_path='../confs', config_name='synthetic_nerf')
def main(conf: DictConfig) -> None:
    ...
    data_path = os.path.join(base_dir, 'data', conf['dataset_name'], conf['task_name'])
    base_exp_dir = os.path.join(base_dir, 'exp', conf['task_name'], conf['exp_name'])
    ...
    make_image_list(data_path, cameras, conf['dataset']['factor'])
